...
import {
  d as DEFAULT_BRIDGE_DESCRIPTOR_ID,
  ...
  s as getAgentationDescriptorId,
  t as LEGACY_OPENCODE_CONFIG_STORAGE_KEY,
} from '../connections-endpoints.eozIpEtq.js';
import {
...
var sidepanel_default = '/sidepanel.C6DQEMLY.css';
//#endregion
...
var SidePanelApp = class SidePanelApp extends i$2 {
  ...
  _isDescriptorConnected(descriptorId) {
    const descriptor = this._connections.getDescriptor(descriptorId);
    return descriptor?.status === 'connected' || descriptor?.status === 'reachable';
  }
  _getActiveAgentationDescriptorId() {
    return this._currentTabId != null ? getAgentationDescriptorId(this._currentTabId) : null;
  }
  connectedCallback() {
    super.connectedCallback();
  ...
  _renderOpencodeTab() {
    ...
    const activeConnectionId = activeSession
      ? getScopedBridgeDescriptorId(activeSession.sessionId)
      : 'opencode-http';
    return b`
      <div
      ...
      >
        <div class="flex flex-col gap-2 p-3 border-b border-base-300 bg-base-100 shrink-0">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-xs font-semibold opacity-70">HTTP</span>
            <code class="text-xs opacity-70">${this._opencodeBaseUrl}</code>
            <connection-status-badge connection-id="opencode-http"></connection-status-badge>
            <span class="text-xs font-semibold opacity-70">Bridge</span>
            <code class="text-xs opacity-70">${this._bridgeBaseUrl}</code>
                ...
                <div class="flex flex-wrap items-center gap-2">
                  ${c(
                    ...
                    (session) => b`
                      ...
                      <connection-status-badge
                        connection-id=${getScopedBridgeDescriptorId(session.sessionId)}
                      ></connection-status-badge>
                    `,
                  )}
                </div>
  ...
  _renderContent() {
    ...
    const defaultBridgeConnected = this._isDescriptorConnected(DEFAULT_BRIDGE_DESCRIPTOR_ID);
    const activeAgentationDescriptorId = this._getActiveAgentationDescriptorId();
    ...
    return b`
      ...
      >
        <button
          class="w-4 h-4 rounded-full shrink-0 flex items-center justify-center ${this._refreshing ? 'bg-base-300' : defaultBridgeConnected ? 'bg-success' : 'bg-error'} hover:opacity-80 transition-all duration-200 cursor-pointer border-none p-0 overflow-hidden"
        ...
        ${
          activeAgentationDescriptorId
            ? b`<connection-status-badge
              connection-id=${activeAgentationDescriptorId}
            ></connection-status-badge>`
            : A
        }
