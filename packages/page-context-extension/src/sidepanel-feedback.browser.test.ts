...
import type { FeedbackAnnotationActionState, FeedbackActionFormMode } from "./sidepanel-feedback.js";
...
describe("updateFeedbackActionStates", () => {
  it("updates state via updater function", () => {
    const current = { "a1": createFeedbackActionState() };
    const result = updateFeedbackActionStates(current, "a1", (s) => ({ ...s, mode: "dismiss" as FeedbackActionFormMode }));
...
describe("feedbackPushAgentBadgeClass", () => {
  ...
  it('returns ghost badge when disabled', () => {
    expect(feedbackPushAgentBadgeClass({ enabled: false, readiness: "disabled", mode: "disabled" })).toContain("ghost");
  });

  it('returns error badge when last launch failed', () => {
    expect(feedbackPushAgentBadgeClass({
      enabled: true, readiness: "ready", mode: "auto",
...
describe("feedbackPushAgentBadgeText", () => {
  ...
  it('returns "disabled" when not enabled', () => {
    expect(feedbackPushAgentBadgeText({ enabled: false, readiness: "disabled", mode: "disabled" })).toBe("disabled");
  });

  it('returns "last launch failed" on failure', () => {
    expect(feedbackPushAgentBadgeText({
      enabled: true, readiness: "ready", mode: "auto",
      lastLaunch: { result: "failed", attemptedAt: "x", annotationId: "y" },
    })).toContain("failed");
  });
  ...
  it('returns "ready" when ready with no launch record', () => {
    expect(feedbackPushAgentBadgeText({ enabled: true, readiness: "ready", mode: "auto" })).toBe("ready");
  });
});
