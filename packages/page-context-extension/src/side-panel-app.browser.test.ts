...
import { buildToolTree, setScopeEnabled, type PageToolPreferences } from "./page-tool-visibility";
...
describe("side-panel-app tools tree interactions", () => {
  ...
  const builtinTools: PageToolSpec[] = [
    ...
    { name: "extension.get_runtime_status", description: "Get runtime status", _bridgeControlTool: true },
    { name: "feedback.get_snapshot", description: "Get feedback snapshot", _bridgeControlTool: true },
  ];
  ...
  it("unchecks all descendant tool checkboxes when a namespace is disabled", async () => {
    ...
    await vi.waitFor(() => {
      expect(element.shadowRoot?.querySelectorAll('input[type="checkbox"]').length).toBeGreaterThan(0);
    });
    ...
    await vi.waitFor(() => {
      ...
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instA" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instB" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "tool", toolName: "beta.instA.run" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "tool", toolName: "beta.instB.inspect" })?.checked).toBe(false);
    });
  ...
  it("re-checks all descendant tool checkboxes when a namespace is re-enabled", async () => {
    ...
    installRuntimeRequestMock({ builtinTools, tabs, pageEntries, initialPreferences: disabledPreferences });
    ...
    await vi.waitFor(() => {
      ...
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instA" })?.checked).toBe(true);
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instB" })?.checked).toBe(true);
      expect(findCheckbox(element, { scope: "tool", toolName: "beta.instA.run" })?.checked).toBe(true);
      expect(findCheckbox(element, { scope: "tool", toolName: "beta.instB.inspect" })?.checked).toBe(true);
    });
  ...
  it("unchecks all namespace and tool checkboxes when a tab is disabled", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(findCheckbox(element, { scope: "namespace", namespace: "alpha" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "namespace", namespace: "beta" })?.checked).toBe(false);
      // All instances should be unchecked
      expect(findCheckbox(element, { scope: "instance", namespace: "alpha", instanceId: "default" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instA" })?.checked).toBe(false);
      expect(findCheckbox(element, { scope: "instance", namespace: "beta", instanceId: "instB" })?.checked).toBe(false);
      // All tools should be unchecked
      expect(findCheckbox(element, { scope: "tool", toolName: "alpha.inspect" })?.checked).toBe(false);
  ...
  it("unchecks all builtin tool checkboxes when builtins are disabled", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(findCheckbox(element, { scope: "builtin", toolName: "feedback.get_snapshot" })?.checked).toBe(true);
    });
  ...
  sendRuntimeRequestMock.mockImplementation(async (method: string, params?: unknown) => {
    switch (method) {
      ...
      case BRIDGE_METHODS.extensionPageToolsSetEnabled: {
        ...
        const scopedEntries = payload.root === "builtin" || payload.tabId == null
          ? undefined
...
function findCheckbox(
  ...
  return checkboxes.find((checkbox) => {
    return Object.entries(expected).every(([key, value]) => checkbox.dataset[key as keyof DOMStringMap] === value);
  }) ?? null;
}
