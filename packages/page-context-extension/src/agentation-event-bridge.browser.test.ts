...
function resolveTargetRect(
  ann: { x?: number; y?: number; boundingBox?: { x: number; y: number; width: number; height: number }; isFixed?: boolean },
...
function buildCreatePayload(
  ann: { comment?: string; severity?: string; element?: string; elementPath?: string; fullPath?: string; reactComponents?: string; sourceFile?: string; isMultiSelect?: boolean; isFixed?: boolean; x?: number; y?: number; boundingBox?: { x: number; y: number; width: number; height: number }; selectedText?: string },
...
describe("Agentation event bridge — end to end", () => {
  ...
  describe("create annotation flow", () => {
    it("full cycle: dispatch → validate → convert → adapter.createAnnotation", async () => {
      ...
      const result = await receiveAndForward("page-context:agentation:annotation:add", dispatchedEvents[0].detail);
      ...
      const callArg = vi.mocked(adapter.createAnnotation)!.mock.calls[0][0] as { body: string; priority: string; target?: { elementName: string; elementPath: string } };
  ...
  describe("update annotation flow", () => {
    it("full cycle: dispatch → validate → convert → adapter.updateAnnotation", async () => {
      ...
      const result = await receiveAndForward("page-context:agentation:annotation:update", dispatchedEvents[0].detail);
      expect(result).not.toContain("rejected");

      expect(vi.mocked(adapter.updateAnnotation)!.mock.calls).toHaveLength(1);
      const callArg = vi.mocked(adapter.updateAnnotation)!.mock.calls[0][0] as { annotationId: string; body: string; priority: string };
  ...
  describe("delete annotation flow", () => {
    it("full cycle: dispatch → validate → convert → adapter.dismissAnnotation", async () => {
      ...
      const result = await receiveAndForward("page-context:agentation:annotation:delete", dispatchedEvents[0].detail);
      expect(result).not.toContain("rejected");

      expect(vi.mocked(adapter.dismissAnnotation)!.mock.calls).toHaveLength(1);
      const callArg = vi.mocked(adapter.dismissAnnotation)!.mock.calls[0][0] as { annotationId: string };
  ...
  describe("event type routing", () => {
    it("only handles known event types, ignores others", async () => {
      ...
      const result = await receiveAndForward("page-context:agentation:annotation:unknown", dispatchedEvents[0].detail);
  ...
  describe("adapter error handling", () => {
    it("propagates createAnnotation errors back without throwing", async () => {
      ...
      let result: unknown;
      try {
        result = await receiveAndForward("page-context:agentation:annotation:add", dispatchedEvents[0].detail);
      } catch (e) {
        result = e;
      }
    ...
    it("propagates updateAnnotation errors back without throwing", async () => {
      ...
      let result: unknown;
      try {
        result = await receiveAndForward("page-context:agentation:annotation:update", dispatchedEvents[0].detail);
      } catch (e) {
        result = e;
      }
      expect(result).toBeInstanceOf(Error);
