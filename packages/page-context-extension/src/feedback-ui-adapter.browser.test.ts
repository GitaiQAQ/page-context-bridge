...
describe('createFeedbackUiAdapter', () => {
  ...
  it('normalizes various create result shapes', async () => {
    ...
    const r1 = await adapter1.createAnnotation({
      body: 'test',
      priority: 'normal',
      target: { elementName: 'el', elementPath: '', rect: new DOMRectReadOnly() },
    });
    expect(r1.id).toBe('anno-a');

    // Shape 2: { annotation: { id: "x" } }
    sendRequest.mockResolvedValueOnce({ annotation: { id: 'anno-b' } });
    const r2 = await adapter1.createAnnotation({
      body: 'test',
      priority: 'normal',
      target: { elementName: 'el', elementPath: '', rect: new DOMRectReadOnly() },
    });
    ...
    const r3 = await adapter1.createAnnotation({
      body: 'test',
      priority: 'normal',
      target: { elementName: 'el', elementPath: '', rect: new DOMRectReadOnly() },
    });
