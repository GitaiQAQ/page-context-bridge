...
describe("createRuntimeMessageHandler", () => {
  ...
  async function createHandler(overrides?: Record<string, unknown>) {
    const deps = {
      ...
      extensionControlHandlers: {
        buildExtensionStatusResponse: vi.fn().mockReturnValue({ connected: true, wsUrl: null, pendingToolCalls: 0, sessionId: "s1" }),
  ...
  describe("delegation to extensionControlHandlers", () => {
    it("routes extensionStatusGet", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: buildStatus, handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionReconnect", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: reconnect, handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionPageToolsGet", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: getPageTools, handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionPageToolsTreeGet", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: getTree, handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionPageToolsDiscover/Refresh", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: refresh, handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionContextManifestGet", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: getManifest, handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
    ...
    it("routes extensionContextResourceRead", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: readRes, handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
      await h({ method: BRIDGE_METHODS.extensionContextResourceRead, params: { tabId: 1, resourceId: "r1" } }, sender);
    ...
    it("routes extensionContextSkillGet", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: getSkill, handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: vi.fn() } as never });
      await h({ method: BRIDGE_METHODS.extensionContextSkillGet, params: { tabId: 1, skillId: "s1" } }, sender);
  ...
  describe("feedback delta validation", () => {
    it("rejects non-finite afterSeq", async () => {
      ...
      await expect(
        h({ method: BRIDGE_METHODS.extensionFeedbackStateDelta, params: { afterSeq: NaN } }, sender),
    ...
    it("accepts afterSeq=0", async () => {
      const requestBridge = vi.fn().mockResolvedValue({});
      const h = await createHandler({ requestBridgeMethod: requestBridge });
      await h({ method: BRIDGE_METHODS.extensionFeedbackStateDelta, params: { afterSeq: 0 } }, sender);
  ...
  describe("feedback annotation create validation", () => {
    it("rejects empty body", async () => {
      const h = await createHandler();
      await expect(
        h({ method: BRIDGE_METHODS.extensionFeedbackAnnotationCreate, params: { body: "   " } }, sender),
    ...
    it("rejects missing priority", async () => {
      const h = await createHandler();
      await expect(
        h({ method: BRIDGE_METHODS.extensionFeedbackAnnotationCreate, params: { body: "test" } }, sender),
  ...
  describe("feedback annotation update validation", () => {
    it("rejects missing annotationId", async () => {
      const h = await createHandler();
      await expect(
        h({ method: BRIDGE_METHODS.extensionFeedbackAnnotationUpdate, params: { body: "test", priority: "medium" } }, sender),
  ...
  describe("feedback annotation dismiss validation", () => {
    ...
    it("rejects empty annotationId after trim", async () => {
      const h = await createHandler();
      await expect(
        h({ method: BRIDGE_METHODS.extensionFeedbackAnnotationDismiss, params: { annotationId: "   " } }, sender),
      ).rejects.toThrow("annotationId is required");
    });
  });
  ...
  describe("set enabled delegation", () => {
    it("delegates to extensionControlHandlers", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: setEnabled, handleExtensionToolDebugCall: vi.fn() } as never });
      await h({
  ...
  describe("debug call delegation", () => {
    it("delegates to extensionControlHandlers", async () => {
      ...
      const h = await createHandler({ extensionControlHandlers: { buildExtensionStatusResponse: vi.fn(), handleExtensionReconnect: vi.fn(), handleExtensionPageToolsGet: vi.fn(), handleExtensionPageToolsTreeGet: vi.fn(), handleExtensionPageToolsRefresh: vi.fn(), handleExtensionContextManifestGet: vi.fn(), handleExtensionContextResourceRead: vi.fn(), handleExtensionContextSkillGet: vi.fn(), handleExtensionPageToolsSetEnabled: vi.fn(), handleExtensionToolDebugCall: debugCall } as never });
      await h({
