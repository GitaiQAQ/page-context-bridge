...
describe('example page', () => {
  ...
  it('routes same-namespace same-tool-name calls to the correct instance', () => {
    ...
    const secondaryGetItems = (
      window as Window & {
        __PAGE_CONTEXT_BRIDGE_TEST__?: (tool: string, args?: Record<string, unknown>) => unknown;
      }
    ).__PAGE_CONTEXT_BRIDGE_TEST__;

    const primary = secondaryGetItems?.('catalog.primary.getItems') as {
      items: string[];
      total: number;
    };
    const secondary = secondaryGetItems?.('catalog.secondary.getItems') as {
      instanceId: string;
      items: string[];
      total: number;
    };
    const summary = secondaryGetItems?.('catalog.secondary.getInstanceSummary') as {
      instanceId: string;
      canSeed: boolean;
    };
  ...
  it('supports built-in tools against the example fixture', () => {
    ...
    ).toMatchObject({ text: 'Selection target text for bridge tests.' });
  });
  ...
  it('exposes manifest, resources, and skill prompts for bridge compilation', () => {
    ...
    const prompt = pageTools.getSkill('form.update-profile', {
      goal: 'Update form and confirm preview',
    });
