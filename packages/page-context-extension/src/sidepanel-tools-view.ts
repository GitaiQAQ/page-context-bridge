...
export function renderToolsTab(input: RenderToolsTabInput): TemplateResult {
  return html`
    <div class="tab-content ${input.active ? "active" : ""} flex flex-col flex-1 min-h-0">
      ...
      <div class="flex-1 overflow-y-auto" id="toolsPanel" @change=${input.onPanelChange} @click=${input.onPanelClick}>
        ...
        ? html`
          <div class="test-panel open border-t border-base-300 bg-base-100 p-3 flex-col gap-2 shrink-0 max-h-[48%] overflow-auto">
            <div class="flex items-center justify-between gap-2">
              ...
              <button class="btn btn-xs btn-ghost" @click=${input.onCloseToolTestPanel}>Close</button>
            </div>
            ...
            <div class="flex flex-col gap-1">
              ...
              <pre class="bg-base-200 rounded-lg p-2 text-xs font-mono whitespace-pre-wrap break-words min-h-[3rem]">${input.toolTestSchemaOutput}</pre>
            </div>
            <div class="flex flex-col gap-1">
            ...
            <div class="flex gap-2 justify-end">
              <button class="btn btn-xs btn-ghost" @click=${input.onResetToolTestArgs}>Reset Args</button>
              <button class="btn btn-xs btn-primary" .disabled=${input.toolTestRunning} @click=${input.onRunToolDebugCall}>Run RPC Call</button>
            </div>
            <div class="text-xs font-semibold ${input.toolTestStatusClass}">${input.toolTestStatusText}</div>
            <div class="flex flex-col gap-1">
              <label class="label text-xs font-semibold" for="toolTestOutput">Output</label>
              <pre class="bg-base-200 rounded-lg p-2 text-xs font-mono whitespace-pre-wrap break-words min-h-[3rem]">${input.toolTestOutput}</pre>
            </div>
          </div>
        `
        : html``}
