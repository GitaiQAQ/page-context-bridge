...
interface FeedbackEvent {
  ...
  annotationId?: string;
  seq: number;
  eventType: "session.started" | "annotation.created" | "annotation.updated" | "annotation.claimed" | "annotation.replied" | "annotation.resolved" | "annotation.dismissed";
  occurredAt: string;
  source: string;
...
describe("feedback-ui-adapter — delta & pushAgent integration", () => {
  ...
  describe("getFeedbackStateDelta — cursor semantics", () => {
    ...
    it("chains multiple deltas with advancing cursor", async () => {
      ...
      expect(sendRequest).toHaveBeenNthCalledWith(1, BRIDGE_METHODS.extensionFeedbackStateDelta, { afterSeq: 0 });
      expect(sendRequest).toHaveBeenNthCalledWith(2, BRIDGE_METHODS.extensionFeedbackStateDelta, { afterSeq: 5 });
    });
  ...
  describe("cursor robustness — edge cases", () => {
    it("falls back to current cursor when delta returns invalid lastSeq", async () => {
      ...
      sendRequest.mockResolvedValueOnce({
        events: [{ eventId: "e1", tenantId: "t1", sessionId: "s1", seq: 10, eventType: "session.started", occurredAt: "", source: "system", payload: {} }],
        lastSeq: 10,
      } satisfies FeedbackStateDeltaResult);
      ...
      sendRequest.mockResolvedValueOnce({
        events: [{ eventId: "e2", tenantId: "t1", sessionId: "s1", seq: 11, eventType: "annotation.created", occurredAt: "", source: "user", payload: {} }],
        lastSeq: 11,
      } satisfies FeedbackStateDeltaResult);
      ...
      expect(sendRequest).toHaveBeenNthCalledWith(3, BRIDGE_METHODS.extensionFeedbackStateDelta, { afterSeq: 10 });
    });
    ...
    it("falls back to current cursor when delta returns non-numeric lastSeq", async () => {
      ...
      expect(sendRequest).toHaveBeenNthCalledWith(2, BRIDGE_METHODS.extensionFeedbackStateDelta, { afterSeq: 0 });
    });
    ...
    it("snapshot resets cursor even if delta had advanced further", async () => {
      ...
      expect(sendRequest).toHaveBeenNthCalledWith(3, BRIDGE_METHODS.extensionFeedbackStateDelta, { afterSeq: 20 });
    });
  });
