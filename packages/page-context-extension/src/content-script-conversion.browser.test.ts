...
describe('content-script agentation conversion functions', () => {
  ...
  describe('resolveTargetRect', () => {
    ...
    it('uses boundingBox when available', () => {
      const rect = resolveTargetRect(
        { boundingBox: { x: 10, y: 20, width: 100, height: 50 }, isFixed: false },
        mockWin,
      );
    ...
    it('adjusts y for fixed elements', () => {
      const rect = resolveTargetRect(
        { boundingBox: { x: 10, y: 100, width: 100, height: 50 }, isFixed: true },
        mockWin,
      );
  ...
  describe('buildCreatePayload', () => {
    ...
    it('uses normal priority for suggestion severity', () => {
      const payload = buildCreatePayload(
        { comment: 'minor issue', severity: 'suggestion' },
        mockWin,
      );
    ...
    it('uses high priority for important severity', () => {
      const payload = buildCreatePayload(
        { comment: 'major issue', severity: 'important' },
        mockWin,
      );
    ...
    it('includes selectedText when present', () => {
      const payload = buildCreatePayload(
        { comment: 'fix this', selectedText: 'error text' },
        mockWin,
      );
    ...
    it('adjusts y coordinate for fixed elements', () => {
      const payload = buildCreatePayload(
        {
          comment: 'fixed elem',
          boundingBox: { x: 0, y: 100, width: 50, height: 50 },
          isFixed: true,
        },
        mockWin,
      );
