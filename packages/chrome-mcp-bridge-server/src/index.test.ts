...
function buildContextResourceName(tabId: number, resource: { namespace: string; id: string }): string {
  return `tab.${tabId}.resource.${resource.namespace}.${resource.id.replace(/[^a-zA-Z0-9._-]/g, "-")}`;
}

function buildContextResourceUri(tabId: number, resource: { namespace: string; id: string }): string {
...
describe("bridge-server: normalizePageToolName", () => {
  ...
  it("only strips namespace prefix when trimmed name re-starts with namespace (double-namespace case)", () => {
    ...
    expect(normalizePageToolName({ name: "catalog.getItems", _namespace: "catalog" })).toBe("catalog.getItems");
  });
  ...
  it("strips namespace prefix in double-namespace case (catalog.catalog.getItems)", () => {
    ...
    expect(normalizePageToolName({ name: "catalog.catalog.getItems", _namespace: "catalog" })).toBe("catalog.getItems");
  });
  ...
  it("handles underscore double-namespace (catalog_catalog.getItems)", () => {
    ...
    expect(normalizePageToolName({ name: "catalog_catalog.getItems", _namespace: "catalog" })).toBe("catalog_catalog.getItems");
  });
  ...
  it("handles namespace_ prefix in trimmed name", () => {
    ...
    expect(normalizePageToolName({ name: "catalog.catalog_getItems", _namespace: "catalog" })).toBe("catalog_getItems");
  });
...
describe("bridge-server: buildContextResourceName", () => {
  it("builds a resource name with tab, namespace, and id", () => {
    expect(
      buildContextResourceName(42, { namespace: "catalog", id: "items" })
    ).toBe("tab.42.resource.catalog.items");
  });

  it("sanitizes special characters in resource id", () => {
    expect(
      buildContextResourceName(1, { namespace: "qa", id: "test/suite" })
    ).toBe("tab.1.resource.qa.test-suite");
  });

  it("preserves dots, dashes, and underscores in resource id", () => {
    expect(
      buildContextResourceName(5, { namespace: "app", id: "my_resource.v2-beta" })
    ).toBe("tab.5.resource.app.my_resource.v2-beta");
  });

  it("handles simple numeric ids", () => {
    expect(
      buildContextResourceName(0, { namespace: "page", id: "summary" })
    ).toBe("tab.0.resource.page.summary");
  });
});
...
describe("bridge-server: buildContextResourceUri", () => {
  it("builds a context URI with tab, namespace, and encoded id", () => {
    expect(
      buildContextResourceUri(42, { namespace: "catalog", id: "items" })
    ).toBe("context://tab/42/resource/catalog/items");
  });

  it("encodes spaces in the id", () => {
    expect(
      buildContextResourceUri(1, { namespace: "qa", id: "test suite" })
    ).toBe("context://tab/1/resource/qa/test%20suite");
  });

  it("encodes special characters", () => {
    expect(
      buildContextResourceUri(3, { namespace: "app", id: "a&b" })
    ).toBe("context://tab/3/resource/app/a%26b");
  });
});
...
describe("bridge-server: buildContextPromptName", () => {
  it("builds a prompt name with tab, namespace, and id", () => {
    expect(
      buildContextPromptName(42, { namespace: "catalog", id: "manage-items" })
    ).toBe("tab.42.skill.catalog.manage-items");
  });

  it("sanitizes special characters in skill id", () => {
    expect(
      buildContextPromptName(1, { namespace: "qa", id: "run/test" })
    ).toBe("tab.1.skill.qa.run-test");
  });

  it("preserves dots, dashes, and underscores", () => {
    expect(
      buildContextPromptName(10, { namespace: "form", id: "update_profile.v2" })
    ).toBe("tab.10.skill.form.update_profile.v2");
  });
});
