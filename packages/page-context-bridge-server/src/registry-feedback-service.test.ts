...
function makeInput(extra?: Partial<CreateRegistryFeedbackServiceInput>): CreateRegistryFeedbackServiceInput {
  return {
    ...
    deriveFeedbackLinks: extra?.deriveFeedbackLinks ?? ((tabId: number) => ({
      links: { namespaceHints: [], relatedToolNames: [], relatedResourceIds: [], relatedSkillIds: [], linkReasons: [] },
      manifest: null,
    })),
    logger: vi.fn(),
  };
