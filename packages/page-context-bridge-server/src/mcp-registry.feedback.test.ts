...
function createRegistry(
  feedbackAgentPushAdapter: FeedbackAgentPushAdapter | null | undefined = null,
...
describe('mcp-registry feedback tools', () => {
  it('derives links from manifest and page tools while creating annotation', () => {
    ...
    const manifest: PageContextManifest = {
      ...
      skills: [
        { id: 'lead.fix', namespace: 'lead', title: 'Lead Fix', description: 'Fix lead issues' },
      ],
  ...
  it('supports full lifecycle through namespaced feedback control tools', async () => {
    ...
    const created = parseTextResponse(createdPayload).annotation as {
      id: string;
      body: string;
      priority: string;
    };
    ...
    const updated = parseTextResponse(updatedPayload).annotation as {
      body: string;
      priority: string;
    };

    const claim = fakeServer.tools.get(FEEDBACK_CONTROL_TOOL_NAMES.claim);
    const claimedPayload = await claim?.({
      annotationId: created.id,
      actorId: 'agent-9',
      actorName: 'Agent Nine',
    });
    ...
    const replied = parseTextResponse(repliedPayload).annotation as {
      thread: Array<{ body: string; kind: string }>;
    };
    ...
    const resolved = parseTextResponse(resolvedPayload).annotation as {
      status: string;
      resolution?: string;
    };
    ...
    const dismissed = parseTextResponse(dismissedPayload).annotation as {
      status: string;
    ...
    const annotations = snapshot.annotations as Array<{
      id: string;
      body: string;
      priority: string;
    }>;
    ...
    expect(
      replied.thread.some((item) => item.body.includes('add logs') && item.kind === 'action_note'),
    ).toBe(true);
