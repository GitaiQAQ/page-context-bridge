...
describe('FeedbackStore - Status Transitions', () => {
  it('allows open -> claimed', () => {
    ...
    const claimed = store.claimAnnotation({
      annotationId: created.id,
      actor: { source: 'agent', id: 'bot', displayName: 'Bot' },
    });
  ...
  it('allows claimed -> in_progress', () => {
    ...
    const updated = store.claimAnnotation({
      annotationId: created.id,
      actor: createDefaultActor(),
    });
  ...
  it('same status transition is no-op', () => {
    ...
    const claimed1 = store.claimAnnotation({
      annotationId: created.id,
      actor: createDefaultActor(),
    });
    const claimed2 = store.claimAnnotation({
      annotationId: created.id,
      actor: createDefaultActor(),
    });
...
describe('FeedbackStore - Reply Thread', () => {
  it('adds reply to annotation thread', () => {
    ...
    const replied = store.replyAnnotation({
      annotationId: created.id,
      actor: createDefaultActor(),
...
describe('FeedbackStore - Update Validation', () => {
  ...
  it('rejects empty body', () => {
    ...
    expect(() =>
      store.updateAnnotation({ annotationId: created.id, actor: createDefaultActor(), body: '' }),
  ...
  it('rejects dismissed annotations', () => {
    ...
    ).toThrow('Cannot update annotation in status: dismissed');
  });
