...
export function startSseServer(mcpHttpPort: number, manager: TenantManager): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    const transports = new Map<string, { transport: SSEServerTransport; server: McpServer; tenantId: string }>();
    ...
    const httpServer = createServer(async (req, res) => {
      ...
      if (req.method === "POST" && (urlPath === "/message" || urlPath === "/message")) {
        ...
        try {
          ...
          log(`[${entry.tenantId}] SSE POST error:`, error instanceof Error ? error.message : String(error));
          if (!res.headersSent) {
      ...
      if (req.method === "GET" && endpoint === "/sse" || endpoint === "sse") {
        ...
        transports.set(`${tenantId}::${transport.sessionId}`, { transport, server: mcpServer, tenantId });
        ...
        try {
          ...
          log(`[${tenantId}] SSE connect error:`, error instanceof Error ? error.message : String(error));
        }
        return;
      }
