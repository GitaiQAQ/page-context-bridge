    ...
    try {
      const registeredTool = mcpServer.registerTool(
        registeredToolName,
        {
          ...
          annotations: {
            readOnlyHint: actualToolName.includes("get_")
        ...
        async (args) => {
          try {
            const result = await rpcCaller.sendToolCall(actualToolName, (args ?? {}) as Record<string, unknown>, tabId);
      ...
      logger("Failed to register page tool", registeredToolName, error instanceof Error ? error.message : String(error));
    }
  }
}
...
export function deletePageTools(input: {
  state: RegistryTabState;
  tabId: number;
}): void {
