...
import {
  ...
  debugToolCallOnExtension,
  ensureMainWorldHostFromBridge,
  ensureAgentationMainFromBridge,
    ...
    return new McpRegistry(
      {
        sendToolCall: (tool, args, tabId) =>
          sendToolCallToExtension(tenantId, tenantManager, tool, args, tabId),
        ...
        debugToolCall: (toolName, args, tabId) =>
          debugToolCallOnExtension(tenantId, tenantManager, toolName, args, tabId),
        ensureMainWorldHost: (tabId, frameId) =>
          ensureMainWorldHostFromBridge(tenantId, tenantManager, tabId, frameId),
        ensureAgentationMain: (tabId, frameId) =>
          ensureAgentationMainFromBridge(tenantId, tenantManager, tabId, frameId),
        getContextManifest: (tabId) =>
          getContextManifestFromExtension(tenantId, tenantManager, tabId),
        getContextManifestDebug: (tabId) =>
          getContextManifestDebugFromExtension(tenantId, tenantManager, tabId),
        refreshPageTools: (tabId) => refreshPageToolsFromExtension(tenantId, tenantManager, tabId),
        readContextResource: (tabId, resourceId) =>
          readContextResourceFromExtension(tenantId, tenantManager, tabId, resourceId),
        getContextSkillPrompt: (tabId, skillId, input) =>
          getContextSkillPromptFromExtension(tenantId, tenantManager, tabId, skillId, input),
        getPageToolsTree: () => getPageToolsTreeFromExtension(tenantId, tenantManager),
        setPageToolsEnabledBatch: (updates) =>
          setPageToolsEnabledBatchOnExtension(tenantId, tenantManager, updates),
      },
      tenantId,
    );
  },
...
async function main(): Promise<void> {
  ...
  if (useSse) {
    ...
    try {
      ...
      log(
        'FATAL: Failed to start MCP server on stdio:',
  ...
  if (!(await startWebSocketServer(EXT_WS_PORT, tenantManager))) {
    log(
      'WARNING: WebSocket server failed to start. MCP tools will return errors until the extension can connect.',
    );
  }
