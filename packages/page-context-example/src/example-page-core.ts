...
export function bootstrapExamplePage(win: Window, doc: Document): ExampleBootstrapResult {
  ...
  globalWindow.__PAGE_CONTEXT_BRIDGE_TEST__ = (tool, args = {}) =>
    invokeToolByQualifiedName(pageTools, tool, args);
...
function createHelpers(win: Window, doc: Document, refs: ExampleDomRefs, state: ExampleState) {
  ...
  const addItem = (text: string) => {
    ...
    button.addEventListener('click', () =>
      removeItemAt(Array.from(refs.itemsList.children).indexOf(item)),
    );
  ...
  const getItems = () =>
    Array.from(refs.itemsList.querySelectorAll('li > span')).map((node) => node.textContent ?? '');
...
function createVersionedPageTools(
  ...
  const namespaceDescriptors: ContextNamespaceDescriptor[] = [
    {
      namespace: 'page',
      title: 'Page',
      description: 'Page-level context inspection and summary',
      tags: ['inspect', 'summary'],
    },
    {
      namespace: 'catalog',
      title: 'Catalog',
    ...
    },
    {
      namespace: 'form',
      title: 'Form',
      description: 'Profile form read and write operations',
      tags: ['form', 'profile'],
    },
    {
      namespace: 'metrics',
  ...
  const buildResources = (): ContextResourceDescriptor[] => [
    {
      id: 'page.summary',
      namespace: 'page',
      title: 'Active Page Summary',
      description: 'Current route, counters, selection and tool registry summary',
    ...
    },
    {
      ...
      description: 'Current profile form values',
      mimeType: 'application/json',
      kind: 'json',
      tags: ['form', 'profile'],
    },
    ...
    switch (id) {
      ...
      case 'metrics.logs':
        return {
          ...
          mimeType: 'application/json',
          text: JSON.stringify(
            { entries: state.logs.slice(-10), total: state.logs.length },
            null,
            2,
          ),
        };
  ...
  const buildSkills = (): ContextSkillDescriptor[] => [
    {
      ...
      description:
        'Summarize the current page state using read-only resources and page inspection tools.',
  ...
  const buildSkillPrompt = (
    ...
    const goal =
      typeof input.goal === 'string' && input.goal.length > 0
        ? input.goal
        : 'Complete the page task for the current skill';
    const prompt = [
  ...
  const pageDefault = createInstance(
    'default',
    () => [
  ...
  const catalogPrimary = createInstance(
    'primary',
    () => [
      descriptor('getItems', 'List current catalog items', {
        properties: { limit: { type: 'number' } },
      }),
      descriptor('addItem', 'Add a catalog item', {
        required: ['text'],
        properties: { text: { type: 'string' } },
      }),
      descriptor('removeItem', 'Remove a catalog item by index', {
        required: ['index'],
      ...
      switch (name) {
        case 'getItems':
          return {
            items: helpers
              .getItems()
              .slice(0, Number(payload.limit ?? refs.itemsList.children.length)),
  ...
  const catalogSecondary = createInstance(
    'secondary',
    () => [
      descriptor('getItems', 'List current catalog items from secondary instance', {
        properties: { limit: { type: 'number' } },
      }),
  ...
  const formProfile = createInstance(
    'profile',
    () => [
  ...
  const metricsDashboard = createInstance(
    'dashboard',
    () => [
      descriptor('getSummary', 'Get dashboard metrics for the example page'),
      descriptor('getRecentLogs', 'Get recent action logs', {
...
function createSelfTestRunner(
  ...
  return () => {
    ...
    check('catalog.secondary.seedItems', () => {
      invokeToolByQualifiedName(pageTools, 'catalog.secondary.seedItems', {
        items: ['alpha', 'beta'],
      });
    ...
    check('catalog.primary.getItems vs catalog.secondary.getItems', () => {
      const primary = invokeToolByQualifiedName(pageTools, 'catalog.primary.getItems') as {
        items: string[];
        total: number;
      };
      const secondary = invokeToolByQualifiedName(pageTools, 'catalog.secondary.getItems') as {
        instanceId: string;
        items: string[];
        total: number;
      };
    ...
    check('form.profile.setProfile', () => {
      invokeToolByQualifiedName(pageTools, 'form.profile.setProfile', {
        fullName: 'Trae QA',
        role: 'reviewer',
      });
    ...
    check('metrics.dashboard.getSummary', () => {
      const summary = invokeToolByQualifiedName(pageTools, 'metrics.dashboard.getSummary') as {
        totalItems: number;
        counter: number;
      };
...
function attachUiHandlers(
  ...
  byId<HTMLButtonElement>(doc, 'counter-inc').addEventListener('click', () =>
    helpers.setCounter(state.counter + 1),
  );
  byId<HTMLButtonElement>(doc, 'counter-dec').addEventListener('click', () =>
    helpers.setCounter(state.counter - 1),
  );
  byId<HTMLButtonElement>(doc, 'counter-reset').addEventListener('click', () =>
    helpers.setCounter(0),
  );
  byId<HTMLButtonElement>(doc, 'counter-set').addEventListener('click', () =>
  ...
  byId<HTMLButtonElement>(doc, 'fixture-reset-btn').addEventListener('click', () =>
    helpers.resetFixture(),
  );
  ...
  byId<HTMLButtonElement>(doc, 'test-page-info').addEventListener('click', () => {
    helpers.showResult(
      true,
      JSON.stringify(invokeToolByQualifiedName(pageTools, 'page.getPageInfo'), null, 2),
    );
  });
  ...
  byId<HTMLButtonElement>(doc, 'test-query-h1').addEventListener('click', () => {
    const results = Array.from(doc.querySelectorAll('h1, h2')).map((element) => ({
      tag: element.tagName,
  ...
  byId<HTMLButtonElement>(doc, 'test-execute-js').addEventListener('click', () => {
    ...
    const result = eval(
      'document.title + " | Items: " + document.querySelectorAll("#items-list li").length + " | Counter: " + window.__testCounter',
    );
...
function invokeToolByQualifiedName(
  pageTools: VersionedPageTools,
  tool: string,
  args: ToolInput = {},
): unknown {
