...
import { FeedbackControlBridgeProvider, FEEDBACK_CONTROL_TOOL_SUFFIXES } from "./feedback-control-bridge-provider.js";

describe("FeedbackControlBridgeProvider", () => {
  function createRpc(overrides?: Record<string, unknown>) {
    return {
      getFeedbackSnapshot: vi.fn().mockResolvedValue({ sessions: [], annotations: [], snapshotVersion: 0 }),
