/**
 * Feedback UI adapter — protocol field mapping + afterSeq cursor maintenance.
 *
 * Fully dependency-injected: sendRequest is provided by the extension runtime
 * (chrome.runtime.sendMessage wrapper). No direct chrome API imports here.
 */
 ...
 * Only responsible for "protocol field mapping + afterSeq cursor maintenance",
 * keeping clear message boundaries between UI layer and runtime.
 */
export function createFeedbackUiAdapter(deps: FeedbackUiAdapterDeps): FeedbackUiAdapter {
  ...
  return {
    async createAnnotation(input: FeedbackUiCreateInput): Promise<FeedbackUiCreateResult> {
      const payload = { body: input.body, priority: input.priority, selectedText: input.selectedText, uiAnchor: input.uiAnchor };
    ...
    async updateAnnotation(input: FeedbackUiUpdateInput): Promise<unknown> {
      const payload = { annotationId: input.annotationId, body: input.body, priority: input.priority };
    ...
    async dismissAnnotation(input: FeedbackUiDismissInput): Promise<unknown> {
      const payload = { annotationId: input.annotationId, dismissReason: input.dismissReason };
...
function normalizeCreateResult(raw: unknown): FeedbackUiCreateResult {
  ...
  if (typeof record.id === "string") return { id: record.id, raw };
  const annotation = record.annotation;
  if (annotation && typeof (annotation as { id?: unknown }).id === "string") {
