...
describe('RpcPeer', () => {
  ...
  describe('receive() - Request Handling', () => {
    it('dispatches to registered handler', async () => {
      ...
      peer.register('test.method', handler);
      await peer.receive(
        JSON.stringify({ jsonrpc: '2.0', id: '1', method: 'test.method', params: { a: 1 } }),
      );

      expect(handler).toHaveBeenCalledWith(
        { a: 1 },
        expect.objectContaining({ id: '1', method: 'test.method' }),
      );
    });
    ...
    it('returns error response for thrown error', async () => {
      ...
      peer.register('test.method', async () => {
  ...
  describe('receive() - Notification Handling', () => {
    it('calls handler without expecting response', async () => {
      ...
      await peer.receive(
        JSON.stringify({ jsonrpc: '2.0', method: 'test.event', params: { x: 1 } }),
      );
    ...
    it('silently ignores unregistered notification methods', async () => {
      ...
      await expect(
        peer.receive(JSON.stringify({ jsonrpc: '2.0', method: 'unknown.event' })),
  ...
  describe('receive() - Response Handling', () => {
    it('resolves pending request with success result', async () => {
      ...
      await peer.receive(
        JSON.stringify({ jsonrpc: '2.0', id: request.id, result: 'response-data' }),
      );
...
describe('Message Helpers', () => {
  ...
  describe('Type Guards', () => {
    ...
    it('isRpcResponse identifies responses (error)', () => {
      expect(isRpcResponse({ jsonrpc: '2.0', id: '1', error: { code: -1, message: 'err' } })).toBe(
        true,
      );
    });
