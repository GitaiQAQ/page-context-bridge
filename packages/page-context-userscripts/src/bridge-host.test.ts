...
import { PAGE_CONTEXT_BRIDGE_HOST_READY_EVENT, getOrCreatePageContextBridgeHost } from "./bridge-host";
...
describe("page context bridge host", () => {
  ...
  it("merges sources and resolves namespace conflicts by priority", () => {
    ...
    const value = JSON.parse(host.bridge.readResource("shared.summary").text) as { from: string; value: string };
  ...
  it("adopts existing bridge and removes adopted alias when same bridge is explicitly registered", () => {
    ...
    host.registerSource({ sourceId: "page-runtime", bridge: foreignBridge, priority: 90, tags: ["page"] });
    ...
    getNamespace: (requestedNamespace: string) => {
      ...
      return {
        ...
        getInstance: (instanceId: string) => {
          ...
          return {
            instanceId,
            listTools: () => [{ name: "inspect", description: "inspect", inputSchema: { type: "object", properties: {}, additionalProperties: false } }],
