...
describe('userscript adapters', () => {
  ...
  it('react adapter supports detect + degrade path', () => {
    ...
    const withReact = instance.callTool('listRoots', {}) as {
  ...
  it('apollo adapter reads cache/query data and degrades when client missing', () => {
    ...
    (window as Window & { __APOLLO_CLIENT__?: unknown }).__APOLLO_CLIENT__ =
      createMockApolloClient();
    const active = instance.callTool('listActiveQueries', {}) as {
      detected: boolean;
      queryCount: number;
    };
  ...
  it('tanstack query adapter reads query/mutation snapshots', () => {
    ...
    (window as Window & { TANSTACK_QUERY_CLIENT?: unknown }).TANSTACK_QUERY_CLIENT =
      createMockTanstackClient();
    const queries = instance.callTool('listQueries', {}) as {
      detected: boolean;
      queryCount: number;
    };
  ...
  it('jotai adapter reads mounted atoms and falls back cleanly', () => {
    ...
    (globalThis as { __JOTAI_DEFAULT_STORE__?: unknown }).__JOTAI_DEFAULT_STORE__ =
      createMockJotaiStore();
    const mounted = instance.callTool('listMountedAtoms', {}) as {
      detected: boolean;
      atomCount: number;
    };
  ...
  it('redux devtools adapter records late extension connect/init/send flow', () => {
    ...
    const initial = instance.callTool('listStores', {}) as {
    ...
    (window as Window & { __REDUX_DEVTOOLS_EXTENSION__?: unknown }).__REDUX_DEVTOOLS_EXTENSION__ =
      extension;
    ...
    subscribers.forEach((listener) =>
      listener({ type: 'DISPATCH', state: JSON.stringify({ count: 2 }) }),
    );

    expect(originalConnect).toHaveBeenCalledTimes(1);
    const stores = instance.callTool('listStores', {}) as {
      storeCount: number;
      stores: Array<{ storeId: string }>;
    };
    expect(stores.storeCount).toBe(1);

    const actions = instance.callTool('listRecentActions', {
  ...
  it('nextjs adapter reads __NEXT_DATA__ and degrades when missing', () => {
    ...
    const peek = instance.callTool('readNextData', { path: 'props.pageProps.user.name' }) as {
      ok: boolean;
      preview: string;
    };
  ...
  it('nuxt adapter reads __NUXT__ and degrades when missing', () => {
    ...
    const peek = instance.callTool('readNuxtPayload', { path: 'state.auth.user.name' }) as {
      ok: boolean;
      preview: string;
    };
    expect(peek.ok).toBe(true);
