...
describe("example page", () => {
  ...
  it("supports built-in tools against the example fixture", () => {
    ...
    expect(executeBuiltInTool("get_page_info", {}, environment)).toMatchObject({ title: "Page Context Bridge - Test Page" });
    expect(executeBuiltInTool("query_elements", { selector: "#items-list li" }, environment)).toMatchObject({ count: 2 });
    expect(executeBuiltInTool("fill_input", { selector: "#full-name-input", value: "Trae QA" }, environment)).toMatchObject({ filled: true });
    expect(executeBuiltInTool("get_element_text", { selector: "#selection-target" }, environment)).toMatchObject({ text: "Selection target text for bridge tests." });
  });
  ...
  it("exposes manifest, resources, and skill prompts for bridge compilation", () => {
    ...
    const prompt = pageTools.getSkill("form.update-profile", { goal: "更新表单并确认预览" });
    expect(prompt?.skill.namespace).toBe("form");
    expect(prompt?.text).toContain("Goal: 更新表单并确认预览");
