...
export function createValuePreview(value: unknown, depth = 0): unknown {
  ...
  if (Array.isArray(value)) {
    return value.slice(0, MAX_ITEMS).map((entry) => createValuePreview(entry, depth + 1)).concat(
  ...
  if (typeof value === "object") {
    ...
    return Object.fromEntries(entries.map(([key, entry]) => [key, createValuePreview(entry, depth + 1)]));
  }
...
export function detectReactEnvironment(target: Record<string, unknown>): ReactEnvironmentSummary {
  ...
  return {
    ...
    reactVersion: renderers.find((renderer) => typeof renderer.version === "string")?.version ?? null,
  };
}
```
...
export function getDisplayName(node: { type?: { displayName?: string; name?: string } } | null | undefined): string {
...
export function listReactRoots(hook: {
  ...
  getFiberRoots?: (rendererId: unknown) => Set<{ current?: { child?: unknown; type?: unknown }; containerInfo?: { tagName?: string } }>;
}): ReactRootSummary[] {
  ...
  for (const rendererId of hook.renderers.keys()) {
    for (const root of hook.getFiberRoots(rendererId)) {
      ...
      summaries.push({
        ...
        displayName: getDisplayName(root.current as { type?: { displayName?: string; name?: string } }),
...
describe("react bridge contract", () => {
  ...
  it("creates readonly react tools", () => {
    const instance = createReactToolInstance({
      listRoots: () => [{ rootId: "root-1", displayName: "App", containerTag: "DIV", childCount: 0 }],
...
export function inspectComponent(node: InspectableFiber | null | undefined) {
  ...
  return {
    ...
    hooks: Array.isArray(node.memoizedStateQueue)
      ? node.memoizedStateQueue.map((entry) => ({ name: entry.name, valuePreview: createValuePreview(entry.value) }))
      : [],
    parentChain,
    children: [],
  };
...
```ts
import type { ContextResourceDescriptor, ContextResourcePayload } from "@page-context/shared-protocol";
...
export function buildReactResources(state: ReactResourceState) {
  const descriptors: ContextResourceDescriptor[] = [
    { id: "react.summary", namespace: "react", title: "React Summary", mimeType: "application/json", kind: "json" },
    { id: "react.roots", namespace: "react", title: "React Roots", mimeType: "application/json", kind: "json" },
    { id: "react.selection", namespace: "react", title: "React Selection", mimeType: "application/json", kind: "json" },
    { id: "react.component", namespace: "react", title: "React Component", mimeType: "application/json", kind: "json" },
    { id: "react.diagnostics", namespace: "react", title: "React Diagnostics", mimeType: "application/json", kind: "json" },
  ];
...
export function createReactToolInstance(env: ReactToolEnv) {
  return {
    ...
    listTools() {
      return [
        { name: "listRoots", description: "List React roots", inputSchema: { type: "object", properties: {} } },
        {
        ...
        { name: "inspectSelectedElement", description: "Inspect the selected DOM element's React component", inputSchema: { type: "object", properties: {} } },
      ];
    },
...
export function buildManifest(input: {
  ...
  return {
    ...
    route: input.route,
    scene: "react-inspection",
    namespaces: [{ namespace: "react", title: "React", description: "React application inspection" }],
...
function createBridge(target: Window & typeof globalThis) {
  ...
  const hook = (target as unknown as { __REACT_DEVTOOLS_GLOBAL_HOOK__?: {
    renderers?: Map<unknown, unknown>;
